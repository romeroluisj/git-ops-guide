...
The `-u` flag sets `origin/main` as the upstream tracking branch so future `git push` and `git pull` need no arguments.
...
| Situation | Default branch |
|---|---|
| New repo you create | `main` (configured in setup) |
| Existing company repo | Check with `git branch -r` — could be `main` or `master` |
