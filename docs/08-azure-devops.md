...
> **The Git commands are the same.** The difference is in the web UI, CI/CD tooling, and access management.
...
| Check for linked work items | PR must reference an Azure Board item | Optional |
...
# Create a PR
az repos pr create --source-branch feature/my-feature --target-branch main --title "feat: my feature"
...
| Task | Azure DevOps | GitHub |
|---|---|---|
| Open a PR | Repos → Pull requests | Pull requests tab (or `gh pr create`) |
| See branch policies | Repos → Branches → Policy | Settings → Branches → Protection |
| Run a pipeline | Pipelines → Run | Actions → Run workflow |
